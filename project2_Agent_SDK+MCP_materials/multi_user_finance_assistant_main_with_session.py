...
load_dotenv()
server_url = os.getenv("server_url") #118.40.235.22
# 全局DeepSeek模型
deepseek_model = None
# 全局会话存储
    ...
    async def initialize(self):
        ...
        self.servers = {
            ...
            "article_check": MCPServerSse(
                ...
                cache_tools_list=True,
                client_session_timeout_seconds=200#这里不是很理解啊
            ),
        ...
        for name, server in self.servers.items():
            try:
                await server.connect()   #主动建立连接   这个是哪里来的
                self.connection_status[name] = True #记录每个mcp服务器的连接状态
...
class FinancialAssistant:
    ...
    async def close_session(self, session_id: str):    #为什么要有这一步呢   主动释放某个用户会话占用的内存
        """关闭会话"""
        if session_id in global_sessions:
            del global_sessions[session_id]

    async def shutdown(self):                        #为什么要有这一步呢
        """关闭服务"""
