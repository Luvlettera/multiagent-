...
from function_utils import exponential_moving_average
#近三日股票服务器的代码实现
import pandas as pd
import asyncio
import numpy as np
...
async def cal_close_price_trend(stock_name):
    if stock_name=="上证指数":
        stock_data = pd.read_excel(
            '/vda2/wangshihang/LLM_api_combine/mcp_lecture_project/project1_stock_counselor/stock_data.xlsx',
            parse_dates=['Date'], sheet_name='上证指数')
    elif stock_name=="中国石油":
        stock_data = pd.read_excel('/vda2/wangshihang/LLM_api_combine/mcp_lecture_project/project1_stock_counselor/stock_data.xlsx',parse_dates=['Date'], sheet_name='中国石油')
    elif stock_name=="中国银行":
        stock_data = pd.read_excel('/vda2/wangshihang/LLM_api_combine/mcp_lecture_project/project1_stock_counselor/stock_data.xlsx',parse_dates=['Date'], sheet_name='中国银行')
    else:
